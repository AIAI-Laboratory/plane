...
@pytest.mark.unit
class TestS3StoragePresignedPUT:
    """Test the presigned PUT URL generation for Cloudflare R2 compatibility"""
    ...
    @patch("plane.settings.storage.boto3")
    def test_default_use_presigned_put_is_false(self, mock_boto3):
        """Test that USE_PRESIGNED_PUT defaults to False"""
        mock_boto3.client.return_value = Mock()
        storage = S3Storage()
        assert storage.use_presigned_put is False
    ...
    @patch("plane.settings.storage.boto3")
    def test_use_presigned_put_enabled(self, mock_boto3):
        """Test that USE_PRESIGNED_PUT=1 enables presigned PUT"""
        mock_boto3.client.return_value = Mock()
        storage = S3Storage()
        assert storage.use_presigned_put is True
    ...
    @patch("plane.settings.storage.boto3")
    def test_generate_presigned_put_returns_url(self, mock_boto3):
        """Test that generate_presigned_put returns a valid URL"""
        mock_s3_client = Mock()
        mock_s3_client.generate_presigned_url.return_value = "https://r2.example.com/test-object?signature=abc"
        mock_boto3.client.return_value = mock_s3_client

        storage = S3Storage()
        result = storage.generate_presigned_put("test-object")

        assert result == "https://r2.example.com/test-object?signature=abc"
        mock_s3_client.generate_presigned_url.assert_called_once()
        call_kwargs = mock_s3_client.generate_presigned_url.call_args[1]
        assert call_kwargs["HttpMethod"] == "PUT"
        assert call_kwargs["Bucket"] == "test-bucket"
        assert call_kwargs["Key"] == "test-object"
    ...
    @patch("plane.settings.storage.boto3")
    def test_generate_presigned_put_with_content_type(self, mock_boto3):
        """Test that generate_presigned_put includes Content-Type when provided"""
        mock_s3_client = Mock()
        mock_s3_client.generate_presigned_url.return_value = "https://r2.example.com/test-object"
        mock_boto3.client.return_value = mock_s3_client

        storage = S3Storage()
        storage.generate_presigned_put("test-object", file_type="image/png")
    ...
    @patch("plane.settings.storage.boto3")
    def test_generate_presigned_put_uses_configured_expiration(self, mock_boto3):
        """Test that generate_presigned_put uses configured expiration"""
        mock_s3_client = Mock()
        mock_s3_client.generate_presigned_url.return_value = "https://r2.example.com/test-object"
        mock_boto3.client.return_value = mock_s3_client

        storage = S3Storage()
        storage.generate_presigned_put("test-object")
    ...
    @patch("plane.settings.storage.boto3")
    def test_generate_presigned_put_handles_client_error(self, mock_boto3):
        """Test that generate_presigned_put handles client errors gracefully"""
